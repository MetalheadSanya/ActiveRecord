...
public extension NSDictionary {
	...
	func swiftCase() -> NSDictionary {
		...
		return self.inflectDictionaryKeys({ (
			string) -> String in string.swiftCase() })
	}

	func rubyCase() -> NSDictionary {

		return self.inflectDictionaryKeys({ (
			string) -> String in string.rubyCase() })
	}

	fileprivate func inflectDictionaryKeys(
		_ closure: @escaping (_ string: String) -> String) -> NSDictionary {
...
public extension String {
	...
	fileprivate func lowerCaseFirstLetter() -> String {
		...
		mutableString.replaceCharacters(in: NSMakeRange(0, 1), with: self.firstLetter()
